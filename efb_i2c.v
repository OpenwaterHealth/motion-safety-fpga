/* Regenerated with UFM enabled so the design has ONE EFB.  The MachXO2 has a
   single Embedded Function Block: map rejects two with
   "Design has 2 EFBs. Device only allows 1 EFB."  Every I2C parameter is
   unchanged from the I2C-only version, I2C1_CLK_DIVIDER = 125 included, so the
   firmware update path over scl_cfg/sda_cfg is untouched.  The only deltas are
   EFB_UFM ENABLED and the wbc_ufm_irq output.
   Regenerate from IPexpress (EFB, with UFM checked), or with the cmd_line in
   efb_i2c.lpc. */
/* Verilog netlist generated by SCUBA Diamond (64-bit) 3.14.0.75.2 */
/* Module Version: 1.2 */
/* C:\lscc\diamond\3.14\ispfpga\bin\nt64\scuba.exe -w -n efb_i2c -lang verilog -synth lse -bus_exp 7 -bb -type efb -arch xo2c00 -freq 50 -i2c1 -i2c1_freq 100 -i2c1_sa 1000001 -i2c1_ce -i2c1_addr 7 -wb -ufm -dev 2000 -e  */
/* Tue Sep 08 12:29:54 2026 */
